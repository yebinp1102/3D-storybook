...
- 프론트엔드
    ...
    - 로그인 성공 )
      ...
      - 이후 유저 권한 식별이 필요한 API 요청을 보낼 때,jwt로 암호화 된 토큰을 localStorage에 저장한다. 이 토큰은 API 요청을 보낼 때마다 header에 넣어서 보냄
    - 로그인 실패 ) 
      - 로그인에 실패했다는 알림창만 등장
...
- 구성
  ...
  - 5가지 메뉴들 
    ...
    5. 로그아웃 : 클릭 시 로그인 할 때 localStorage에 저장했던 accessToken 값을 삭제하고 로그인 페이지로 라우팅
