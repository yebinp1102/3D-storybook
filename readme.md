...
#### [ <span style="color: orange">상품 상세 페이지</span> ]
- 프론트엔드
  - 사용자가 상세 페이지 방문 시 url에 해당 상품의 id값이 담겨 있다. 이 id값에 해당하는 상품의 모든 정보를 요청하는 API가 호출된다.
  - 응답을 받으면, 받은 데이터를 기반으로 UI를 렌더링
  - 사용자가 '지금 보기' 버튼을 누르면 해당 템플릿 시현 페이지로 이동
  - 사용자가 '카트에 담기' 버튼을 누르면 백엔드에게 해당 템플릿을 장바구니에 담아달라는 요청을 보낸다.
  - 응답값 200일 경우 장바구니에 담았다는 알림창을 띄우고, 메뉴바의 장바구니 버튼 옆 숫자를 +1 한다. 그리고 장바구니와 관련된 쿼리(cart 아이템 상세 정보들을 불러오는 API)를 refetch
  - 응답값이 500일 경우 이미 중복이라는 알림창을 띄우기만 한다. 

- 백엔드
  - 먼저 해당 템플릿이 이미 장바구니에 있는지 식별한다. 이때 식별 기준은 템플릿의 id값이다.
  - 장바구니에 이미 있다면 500 상태값과 '이미 저장된 템플릿입니다.'라는 메세지를 보낸다.
  - 장바구니에 없다면, DB에서 해당 유저의 cart 배열에 해당 템플릿의 id값을 넣고 200 상태값을 전달한다. 
  
<br><img src="frontend/public/readme/장바구니_담기.gif" alt="장바구니 담기" width="600" /><figcaption>장바구니 담기</figcaption><br>
<br><img src="frontend/public/readme/장바구니_담기_오류.gif" alt="장바구니 담기 오류" width="600" /><figcaption>장바구니 담기 중복</figcaption><br>

#### [ <span style="color: orange">장바구니 페이지</span> ]
- 프론트엔드
  - userContext에 저장된 유저 정보에서 카트에 담긴 상품(=템플릿) 정보를 기반으로 총액을 계산하고 상품 리스트를 보여준다.
  - 만약 카트에 아무것도 담지 않았다면, 상품 목록 대신 '아직 카트에 담긴 템플릿이 없습니다.'라는 텍스트 렌더링.
  - '주문하기' 버튼을 누르면 주문 정보 입력 페이지로 이동.
  - 상품 리스트 UI에서 X 버튼을 누르면 장바구니에서 해당 상품을 빼달라는 요청을 하는 API 호출.
  - 200 응답값을 받았다면 '해당 템플릿을 카트에서 제거했습니다.'라는 알림창을 띄움. 그리고 장바구니 관련된 쿼리(cart 아이템 상세 정보들을 불러오는 API)를 refetch
  - 500 응답값 받으면, '해당 템플릿을 카트에서 제거하는데 실패했습니다. 다시 시도해주세요.'라는 알림창 띄움.

- 백엔드
  - 장바구니에서 특정 아이템을 빼달라는 요청을 받으면, 해당 id값을 유저의 cart 배열에서 뺀다.
  - 성공 시, 응답값 200 반환
  - 실패 시, 응답값 500 반환 
  
<br><img src="frontend/public/readme/장바구니_담기.gif" alt="장바구니 담기" width="600" /><br>
