...
const TemplateList = ({ temp, handleHover, idx }: Props) => {
  ...
  return (
    ...
    >
      <ImageLoad
        bgCSS="blur-load w-full h-full absolute"
        bgImg={temp.images[1]}
        imgCSS="w-full h-full object-cover object-center"
        img={temp.images[0]}
      />

      {!temp.isAvailable && (
        <h1 className="absolute bg-gray-900 text-white bg-opacity-80 w-full h-full flex-center">
          Comming Soon
        </h1>
      )}
