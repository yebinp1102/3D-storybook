      ...
      <div className="max-w-7xl mx-auto px-8 pb-20 flex flex-col">
        ...
        <div className='flex justify-end'>
          <div className='bg-white font-bold mt-8 shadow-lg border rounded-lg p-4 min-w-[30rem]'>
            <p className='border-b-2 border-slate-300 text-xl py-4'>최종 결제 금액 :</p>
            <ul className='py-6 border-b-2 border-slate-300 flex flex-col gap-6'>
              <PriceList text='주문금액 총합' price={`${Number(24600).toLocaleString()}`} />
              <PriceList text='배송비' price={`+ ${Number(3000).toLocaleString()}`} />
              <PriceList text='할인금액' price={`- ${Number(0).toLocaleString()}`} />
